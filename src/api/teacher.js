...
router.get('/', async (req, res, next) => {
    const abc = await con.connect(function (err) {
        con.query(`SELECT * FROM school_db.teacher_master`, function (err, result, fields) {
            if (err) res.send(err);
...
router.get('/group', async (req, res, next) => {
    ...
    const item = await con.connect(function (err) {
        con.query(queryCheck,
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        const item = await con.connect(function (err) {
            con.query(`SELECT * FROM school_db.teacher_master where teacher_id='` + id + "'",
                function (err, result, fields) {
                    if (err) res.send(err);
                    if (result) res.send(result);
                });
...
router.post('/', async (req, res, next) => {
    ...
    const abc = await con.connect(function (err) {
        con.query(`INSERT INTO school_db.teacher_master (teacher_id,first_name,last_name,email,gender,mobile,date_of_birth) VALUES
        ('${uniqueID}','${req.body.first_name}','${req.body.last_name}',
        '${req.body.email}','${req.body.gender}',${req.body.mobile},'${req.body.dob}')`,
            function (err, result, fields) {
                if (err) res.send(err);
            ...
            con.query(`SELECT * FROM school_db.teacher_master where teacher_id='` + id + "'",
                function (err, result, fields) {
                    ...
                    if (result && result.length)
                        con.query(`UPDATE school_db.teacher_master SET first_name = '${req.body.first_name}', 
                        last_name = '${req.body.last_name}', email = '${req.body.email}', 
                        gender = '${req.body.gender}', mobile = '${req.body.mobile}', date_of_birth = '${req.body.dob}' 
                        where teacher_id = '${id}'`,
